...
def get_prioritized_models():
    ...
    try:
        ...
        # LOGIC: Sort your specific list by reliability
        # 1. 2.5 Flash (Fastest)
        # 2. 2.5 Lite (Backup Quota)
        # 3. 3.0 Pro (High Quality Fallback)
        
        def sort_key(name):
            if "gemini-2.5-flash" in name and "lite" not in name: return 0
            if "gemini-2.5-flash-lite" in name: return 1
            if "gemini-3" in name: return 2
            if "gemini-2.5" in name: return 3
            return 99 # Legacy stuff
...
# This function loops through your models until one works!
def generate_lesson_with_fallback(image_bytes, lang_config, models_list):
    
    # Try up to 3 different models from your list
    for attempt, model_name in enumerate(models_list[:3]): 
        ...
        try:
            ...
            # IF BUSY (429): Loop will continue to next model
            if response.status_code == 429:
                status_placeholder.warning(f"⚠️ `{model_name}` is busy. Switching engines...")
                time.sleep(1)
    ...
    return None, "All available models are currently busy. Please try again in 1 minute.", "All"
